...
- A comparison is `PRICE_ONLY` only when reference analysis/fingerprint, complete
  assumption identity, input scope, and IG policy are unchanged. Policy changes are
  reported separately.
- Evaluation schema v0.2 records financial unit, accounting scope, and share-basis
  version. Older records remain readable but missing comparison evidence is unresolved.
- Valuation fingerprint v1 is generated by the actual builder from period/ticker and
  EPS or revenue/share inputs. It excludes price, execution timestamps, assumptions, and
  random IDs.
