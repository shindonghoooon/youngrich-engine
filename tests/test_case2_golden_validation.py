...
def build_input(data: dict) -> Case2AnalysisInput:
    ...
    return Case2AnalysisInput(
        ...
        valuation_assumptions=assumptions, current_market_cap=market_cap, current_price=data["market"]["close"], current_revenue=latest.revenue, current_share_count=data["market"]["shares_for_market_cap"], required_return=data["valuation"]["required_return"],
        valuation_evidence=ValuationEvidenceState(credible_evidence_count=data["valuation"]["credible_evidence_count"], company_economics_stable=data["valuation"]["economics_stable"], company_economics_rapidly_changing=data["valuation"]["economics_rapidly_changing"], available_at=as_of), asymmetry_type=AsymmetryType(data["valuation"]["asymmetry_type"]),
    )
